...
| `03-potential-options.md` | Registry for every considered option: routes, places, lodging, logistics, booking guardrails, split-family options, rejected/parked/retained/selected/visited options. Keeps interest scores, current-fit notes, decision triggers, do-not-book guardrails. Plan status `Visited` marks an option the family has already done; do not schedule it into a later day card. |
