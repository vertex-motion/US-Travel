...
- Drive legs must connect only real points of the day. Every endpoint in `days[].driveLegs` and every point in the `04-daily-itinerary.md` `Driving:` line must be one of: the day's origin base, a stop that appears in that day's `places[]` (with its own parking), or the destination hotel/base. Must not name a pass-through town, junction, or waypoint you only drive through (e.g. Santa Cruz, Barstow) as a leg endpoint — that implies a stop the traveler cannot make. Collapse such a leg into a single origin → destination pair; the dashboard still computes the true driving distance. When you add, remove, rename, or reorder a stop, update the drive legs in both files in the same change.
